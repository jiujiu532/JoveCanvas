...
const KNOWN_ERROR_MESSAGE_ENTRIES: Array<[string, string]> = [
    ...
    ["图片尺寸参数无效", "tasks.imageSizeInvalid"],
    ["图片生成超时，请稍后重试", "tasks.imageTimeoutRetry"],
    ["图片接口返回了无效 JSON，创建结果待确认", "tasks.imageInvalidJsonPending"],
    ["图片接口没有返回图片或任务 ID，创建结果待确认", "tasks.imageNoResultPending"],
    ["图片任务创建结果未知", "tasks.imageCreationUnknown"],
    ["OpenAI 视频协议最多支持 1 张参考图", "tasks.openaiVideoOneRef"],
    ["自定义图片协议缺少创建路径、请求模板或结果字段", "tasks.customImageMissingConfig"],
    ["自定义图片接口调用失败", "tasks.customImageCallFailed"],
    ["自定义图片接口返回失败", "tasks.customImageFailed"],
    ["自定义图片接口没有返回图片或任务 ID，创建结果待确认", "tasks.customImageNoResult"],
    ["自定义图片任务查询失败", "tasks.customImageQueryFailed"],
    ["自定义图片任务完成但没有返回图片", "tasks.customImageDoneNoImage"],
    ["自定义图片任务生成超时", "tasks.customImageTimeout"],
    ["请求体过大", "common.requestBodyTooLarge"],
    ...
    // high-frequency service throws localized at route catch
    ["创作会话不存在", "creative.sessionNotFound"],
    ["创作入口不正确", "creative.surfaceInvalid"],
    ["工作台类型不正确", "creative.workbenchTypeInvalid"],
    ["归档会话不能追加消息", "creative.archivedCannotAppend"],
    ["创作资产不存在", "creative.assetNotFound"],
    ["画布项目不存在", "canvas.projectNotFound"],
    ["画布项目数据过大", "canvas.projectTooLarge"],
    ["画布项目已存在", "canvas.projectAlreadyExists"],
    ["公共提示词备份格式不正确", "backup.promptsFormatInvalid"],
    ["生成日志备份格式不正确", "backup.generationLogsFormatInvalid"],
    ["注销申请备份格式不正确", "backup.accountDeletionFormatInvalid"],
    ["短剧项目不存在", "drama.projectNotFound"],
    ["短剧项目数据过大", "drama.projectTooLarge"],
    ["短剧版本不存在", "drama.versionNotFound"],
    ["素材不存在", "library.assetNotFound"],
    ["已有待处理的注销申请", "auth.deletionPendingExists"],
    ["请输入当前密码", "auth.currentPasswordRequired"],
    ["注销申请已受理，正在处理中", "auth.deletionAcceptedProcessing"],
    ["没有可撤回的待处理申请", "auth.noPendingDeletionToWithdraw"],
    ["请填写处理备注", "auth.reviewNoteRequired"],
    ["申请不存在或已处理", "auth.deletionRequestNotFoundOrHandled"],
    ["图片任务不存在或已过期", "tasks.imageNotFoundOrExpired"],
    ["音频任务不存在或已过期", "tasks.audioNotFoundOrExpired"],
    ["视频任务不存在或已过期", "tasks.videoNotFoundOrExpired"],
    ["自定义文本任务查询失败", "tasks.customTextQueryFailed"],

    // Seedance 2.0 special request validation (lib/seedance-special.ts)
    ["Seedance 2.0 特价版模型不在接口文档允许列表中", "tasks.seedance.modelNotAllowed"],
    ["Seedance 2.0 特价版时长必须是 4-15 秒整数", "tasks.seedance.durationRange"],
    ["Seedance 参考音频不能单独使用，请同时添加参考图片或参考视频", "tasks.seedance.audioNeedsVisual"],
    ["当前 Seedance 模型要求至少一个参考视频", "tasks.seedance.videoRefRequired"],
    ["使用参考视频时必须选择名称以 _with_video_ref 结尾的 Seedance 模型", "tasks.seedance.videoRefModelRequired"],
    ["Seedance 2.0 特价版必须填写文本提示词", "tasks.seedance.promptRequired"],
    ["Seedance 中文提示词不能超过 500 字", "tasks.seedance.zhPromptTooLong"],
    ["Seedance 英文提示词不能超过 1000 词", "tasks.seedance.enPromptTooLong"],
    ["参考图片只能使用公网 URL 或 assetId:// 素材，不能使用 base64", "tasks.seedance.refImagesUrlOnly"],
    ["参考视频只能使用公网 URL 或 assetId:// 素材，不能使用 base64", "tasks.seedance.refVideosUrlOnly"],
    ["参考音频只能使用公网 URL 或 assetId:// 素材，不能使用 base64", "tasks.seedance.refAudiosUrlOnly"],
];
