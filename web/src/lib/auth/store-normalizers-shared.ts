...
export function repairKnownMojibakeText(value: string) {
    if (value.includes("JoveCanvas") && value.includes("AI") && !value.includes("绘图") && value.includes(",")) return DEFAULT_SITE_SETTINGS.seoKeywords;
    if (value.includes("JoveCanvas") && value.includes("AI") && !value.includes("工作台")) return DEFAULT_SITE_SETTINGS.seoDescription;
    if (value.includes("2026 JoveCanvas") && !value.startsWith("©")) return "© 2026 JoveCanvas. All rights reserved.";
