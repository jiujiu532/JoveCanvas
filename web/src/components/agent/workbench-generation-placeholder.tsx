...
export function WorkbenchGenerationActivity({ kind, count }: { kind: "image" | "video"; count: number }) {
    const t = useTranslations("layout");
    const label = t("agent.generationPlaceholder.tasksGenerating", {
        count,
        kind: kind === "image" ? t("agent.generationPlaceholder.kindImage") : t("agent.generationPlaceholder.kindVideo"),
    });
    return (
