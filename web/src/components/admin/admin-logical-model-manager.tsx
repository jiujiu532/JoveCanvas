...
import { deriveLogicalModelsConfig, isLogicalModelResolvable, normalizeDefaultModelsConfig, resolveLogicalModelConfig } from "@/lib/model-routing-config";
...
export function AdminLogicalModelManager({ channels, logicalModels, defaultModels, onChange }: Props) {
    ...
    const syncChannelModels = () => {
        ...
        onChange({ logicalModels: nextModels, defaultModels: normalizeDefaultModelsConfig(defaultModels, nextModels, channels) });
        message.success(t("logicalModels.synced", { count: additions.length }));
    };
                ...
                <div>
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Tag color={readyCount === defaultFields.length ? "green" : "orange"} className="m-0">
                            {t("logicalModels.readyCount", { ready: readyCount, total: defaultFields.length })}
                        </Tag>
                    </div>
            ...
            <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
                <div className="min-w-0">
                    <div className="mb-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_160px]">
                        ...
                        <Select value={capabilityFilter} options={[{ label: t("logicalModels.allCapabilities"), value: "all" }, ...capabilityOptions]} onChange={(value) => setCapabilityFilter(value)} />
                    </div>
...
function capabilityText(capability: LogicalModelCapability, t: AdminTranslator) {
    return t(`logicalModels.capability.${capability}`);
}
