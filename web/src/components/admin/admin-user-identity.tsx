...
export function AdminAccountId({ accountId, className = "" }: { accountId?: string; className?: string }) {
    ...
    // Keep "ID：" prefix stable for tests and dense admin tables; it is a technical label, not prose copy.
    return (
        <span className={`inline-flex min-w-0 items-baseline whitespace-nowrap text-xs leading-5 ${className}`} title={`ID：${displayAccountId}`} aria-label={`ID ${displayAccountId}`}>
...
export function AdminUserIdentity({ displayName, username, accountId, avatarUrl, fallback, className = "" }: { displayName?: string; username?: string; accountId?: string; avatarUrl?: string; fallback?: string; className?: string }) {
    const t = useTranslations("admin");
    const primary = displayName || username || fallback || t("userIdentity.unknownUser");
    return (
...
export function AdminUserSearchSelect({ value, onChange, activeOnly = false, placeholder, className = "w-full" }: { value?: string; onChange?: (value?: string) => void; activeOnly?: boolean; placeholder?: string; className?: string }) {
    ...
    const load = useCallback(
        async (keyword = "") => {
            ...
            try {
                ...
                if (!response.ok || !payload?.users) throw new Error(t("userIdentity.loadFailed"));
    ...
    return (
        <Select
            ...
            notFoundContent={failed ? t("userIdentity.loadFailed") : loading ? t("userIdentity.searching") : t("userIdentity.notFound")}
            options={options}
            optionRender={(option) => {
                ...
                return (
                    <div className="min-w-0 py-0.5">
                        ...
                        <div className="mt-0.5 truncate text-xs text-zinc-500 dark:text-zinc-400">
                            ...
                            {user.status !== "active" ? ` · ${t("userIdentity.disabled")}` : ""}
                        </div>
                    </div>
                );
            }}
