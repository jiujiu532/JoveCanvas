...
export function adminSections(t: ReturnType<typeof useTranslations<"admin">>): AdminSection[] {
    return [
        ...
        { key: "accountDeletion", label: t("nav.sections.accountDeletion.label"), description: t("nav.sections.accountDeletion.description"), shortDescription: t("nav.sections.accountDeletion.shortDescription"), icon: <UserRoundX className="size-4" /> },
        ...
        { key: "externalStorage", label: t("nav.sections.externalStorage.label"), description: t("nav.sections.externalStorage.description"), shortDescription: t("nav.sections.externalStorage.shortDescription"), icon: <Cloud className="size-4" /> },
        ...
        { key: "announcements", label: t("nav.sections.announcements.label"), description: t("nav.sections.announcements.description"), shortDescription: t("nav.sections.announcements.shortDescription"), icon: <Megaphone className="size-4" /> },
...
function buildAdminSectionGroups(t: ReturnType<typeof useTranslations<"admin">>): AdminSectionGroup[] {
    const sections = adminSections(t);
    const sectionsFor = (keys: AdminSectionKey[]) => {
        const keySet = new Set(keys);
        return sections.filter((section) => keySet.has(section.key));
    };
    return [
        { title: t("nav.groups.businessAnalytics"), items: sectionsFor(["overview", "users", "logs", "generationOperations"]) },
        { title: t("nav.groups.productOperations"), items: sectionsFor(["products", "orders"]) },
        { title: t("nav.groups.finance"), items: sectionsFor(["points", "payments", "cdk", "wallet"]) },
        { title: t("nav.groups.upstreamConfig"), items: sectionsFor(["channels", "skills"]) },
        { title: t("nav.groups.system"), items: sectionsFor(["site", "settings", "accountDeletion", "updates"]) },
        { title: t("nav.groups.storageBackup"), items: sectionsFor(["mediaStorage", "externalStorage", "backup"]) },
        { title: t("nav.groups.contentOperations"), items: sectionsFor(["announcements", "prompts"]) },
    ];
}
