...
export function UpdateCenterPanel() {
    ...
    const releaseLinks = repositoryUrl
        ? [
              { label: "Release", href: `${repositoryUrl}/releases`, description: t("updates.releaseLinks.release") },
              { label: "Issues", href: `${repositoryUrl}/issues`, description: t("updates.releaseLinks.issues") },
              { label: "Docs", href: repositoryUrl, description: t("updates.releaseLinks.docs") },
          ]
        : [];
    const upgradeChecks = [t("updates.checklist.backupDb"), t("updates.checklist.envVars"), t("updates.checklist.changelog"), t("updates.checklist.rollback")];
    return (
        <Panel>
            <PanelHeader title={t("nav.sections.updates.label")} description={t("updates.description")} actions={<GitHubLink className="rounded-xl" />} />
            <div className="space-y-3 p-3 sm:space-y-5 sm:p-5">
                <div className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_360px]">
                    <div className="overflow-hidden rounded-3xl border border-stone-200/80 bg-stone-950 text-white shadow-sm shadow-stone-950/10 dark:border-stone-800 dark:bg-white dark:text-stone-950">
                        <div className="grid gap-3 p-4 sm:gap-6 sm:p-6 lg:grid-cols-[minmax(0,1fr)_220px]">
                            <div className="min-w-0">
                                <div className="text-xs font-semibold uppercase tracking-[0.16em] text-white/55 dark:text-stone-500">{site.title} Update</div>
                            ...
                            <div className="rounded-xl bg-white/10 p-3 ring-1 ring-white/12 sm:rounded-2xl sm:p-4 dark:bg-stone-950/5 dark:ring-stone-200">
                                <div className="text-xs text-white/55 dark:text-stone-500">{t("updates.hero.currentVersion")}</div>
                    ...
                    <div className="rounded-xl border border-stone-200/80 bg-stone-50/70 p-3 sm:rounded-3xl sm:p-5 dark:border-stone-800 dark:bg-stone-900/45">
                        <div className="flex items-center justify-between gap-3">
                            <div>
                                <div className="text-sm font-semibold text-stone-950 dark:text-stone-100">{t("updates.checklist.title")}</div>
                                <div className="mt-1 text-xs leading-5 text-stone-500 dark:text-stone-400">{t("updates.checklist.description")}</div>
                            </div>
                ...
                {releaseLinks.length ? (
                    ...
                    <div className="rounded-xl border border-dashed border-stone-200/80 bg-stone-50/60 p-4 text-center text-xs text-stone-500 sm:rounded-3xl sm:p-6 sm:text-sm dark:border-stone-800 dark:bg-stone-900/30 dark:text-stone-400">
                        {t("updates.noRepository")}
                    </div>
                )}
            </div>
        </Panel>
