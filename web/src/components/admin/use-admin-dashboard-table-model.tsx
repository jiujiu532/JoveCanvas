...
export function useAdminDashboardTableModel({ state, data, settingsActions }: { state: AdminDashboardState; data: AdminDashboardDataActions; settingsActions: AdminDashboardSettingsActions }) {
    ...
    const userColumns: TableColumnsType<PublicUser> = [
            ...
            render: (_, record) => (
                <div className="min-w-0">
                    ...
                    <div className="mt-1.5 flex min-w-0 flex-wrap items-center gap-x-2 gap-y-1 sm:hidden">
                        ...
                        <span className="truncate text-xs text-zinc-400">{record.email || t("users.table.noEmail")}</span>
                    </div>
        ...
        },
        {
            ...
            width: 220,
            responsive: ["sm"],
            render: (email?: string) => <span className="block truncate text-sm text-zinc-600 dark:text-zinc-300">{email || t("users.table.noEmail")}</span>,
        },
        {
    ...
    const generationLogColumns: TableColumnsType<StoredGenerationLog> = [
        ...
        },
        {
            ...
            width: 200,
            render: (_, record) => <AdminUserIdentity displayName={record.displayName} username={record.username} accountId={record.accountId} fallback={t("logs.table.userUnavailable")} />,
        },
        {
    ...
    const activeSectionInfo = sections.find((section) => section.key === activeSection) || sections[0];
