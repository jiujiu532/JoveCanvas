...
export function AdminLocalMediaStorage() {
        ...
        () => [
            {
                ...
                render: (_, asset) => (
                    <div className="flex min-w-0 items-center gap-3">
                        <MediaThumbnail asset={asset} onPreview={setPreviewAsset} t={t} />
            ...
            },
            {
                ...
                render: (_, asset) => (
                    <div className="min-w-0 text-xs text-zinc-500">
                        <div className="truncate text-zinc-800 dark:text-zinc-200">{asset.ownerDisplayName || asset.ownerUsername || asset.ownerUserId || t("localMedia.table.unregisteredUser")}</div>
    ...
    return (
        <Panel>
                        ...
                        <div key={asset.id} className="flex min-w-0 items-center gap-3 rounded-md border border-zinc-200 p-3 dark:border-zinc-800">
                            ...
                            <div className="min-w-0 flex-1">
                                ...
                                <div className="mt-1 truncate text-xs text-zinc-500">
                                    {asset.ownerDisplayName || asset.ownerUsername || asset.ownerUserId || t("localMedia.table.unregisteredUser")} · {mediaSourceLabel(asset.source)}
                                </div>
            ...
            >
                {previewAsset ? <MediaViewer asset={previewAsset} t={t} /> : null}
            </Modal>
        </Panel>
    );
}

function MediaThumbnail({ asset, onPreview, t }: { asset: LocalMediaAsset; onPreview: (asset: LocalMediaAsset) => void; t: AdminTranslator }) {
    return (
        <button
            type="button"
...
function MediaViewer({ asset, t }: { asset: LocalMediaAsset; t: AdminTranslator }) {
    if (asset.type === "image")
        return (
