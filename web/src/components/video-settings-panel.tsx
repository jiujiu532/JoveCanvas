...
export function VideoSettingsPanel({ config, onConfigChange, theme, showTitle = true, className = "w-[320px] space-y-4 rounded-2xl px-1 py-0.5" }: VideoSettingsPanelProps) {
    ...
    const sizeOptions = sizeOptionDefs.map((item) => ({
        ...item,
        label: t(`settings.video.${item.labelKey}`),
    }));
...
export function videoSizeLabel(value: string, labels?: { adaptive: string; landscape: string; portrait: string; square: string; wide: string; tall: string }) {
    const map = labels || { adaptive: "Adaptive", landscape: "Landscape", portrait: "Portrait", square: "Square", wide: "Wide", tall: "Tall" };
...
export function videoSecondsLabel(value: string, smartLabel = "Smart") {
