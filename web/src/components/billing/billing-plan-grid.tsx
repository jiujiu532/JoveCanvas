    ...
    return (
        ...
        >
            <div className="relative flex items-center justify-between gap-3">
                <span className="text-[10px] font-semibold tracking-[0.14em] text-stone-400 sm:text-[11px] sm:tracking-[0.16em] dark:text-stone-500">{t("billing.brandPass", { index: String(index + 1).padStart(2, "0") })}</span>
                {promotion ? (
            ...
            {promotion ? (
                <div className="relative -mt-0.5 mb-1 flex items-center gap-2 text-xs text-stone-500 sm:mb-0 sm:mt-1 dark:text-stone-400">
                    <span className="line-through">{t("billing.listPrice", { amount: formatYuan(pricing.listUnitAmountCents) })}</span>
                    <span className="font-medium text-rose-600 dark:text-rose-300">{t("billing.saveAmount", { amount: formatYuan(pricing.discountCents) })}</span>
                </div>
            ) : null}
