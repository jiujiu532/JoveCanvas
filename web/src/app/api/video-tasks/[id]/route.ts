...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
    ...
    if (!body.status || !canTransitionVideoTask(task.status, body.status)) return NextResponse.json({ error: await serverMessage("tasks.statusImmutable") }, { status: 409 });
    ...
    if (!next) return NextResponse.json({ error: await serverMessage("tasks.statusImmutable") }, { status: 409 });
    if (status === "cancelled") {
