...
async function serveAvatar(request: Request, context: Context) {
    ...
    if (new URL(request.url).searchParams.get("download")) return await notFound();
    const rate = await checkPublicMediaRateLimit(`avatar:${userId}`, request);
    if (!rate.allowed) return NextResponse.json({ code: 429, data: null, msg: await serverMessage("media.avatarAccessRateLimited") }, { status: 429, headers: rateLimitHeaders(rate) });

    const storageKey = await getPublicAvatarStorageKey(userId);
    if (!storageKey) return await notFound();
    const registration = await getLocalMediaRegistration(storageKey);
    if (!registration || registration.ownerUserId !== userId || registration.source !== "profile-avatar" || registration.storageClass !== "permanent" || registration.mimeType !== "image/webp") return await notFound();
    ...
    if (!permit) return NextResponse.json({ code: 429, data: null, msg: await serverMessage("media.avatarConcurrencyLimited") }, { status: 429, headers: { "Retry-After": "2" } });
    try {
        if (registration.storageProvider === "object") {
            const url = await createExternalMediaReadUrl(request, registration);
            permit.release();
            if (!url) return await notFound();
        ...
        return NextResponse.json({ code: 500, data: null, msg: await serverMessage("media.avatarReadFailed") }, { status: 500 });
    }
}
...
async function notFound() {
    return NextResponse.json({ code: 404, data: null, msg: await serverMessage("media.avatarNotFound") }, { status: 404 });
}
