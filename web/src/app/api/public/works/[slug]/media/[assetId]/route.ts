...
async function servePublicWorkMedia(request: Request, context: Context) {
    ...
    if (new URL(request.url).searchParams.get("download") === "original") return NextResponse.json({ code: 403, data: null, msg: await serverMessage("media.publicWorkNoOriginalDownload") }, { status: 403 });
