...
export async function POST(request: Request, context: Context) {
    ...
    try {
        return await workPublicationOk(await setWorkAuthorFollow(user.id, (await context.params).slug, body.active), body.active === false ? "已取消关注" : "已关注作者");
    } catch (error) {
        return await workPublicationError(error, "关注操作失败", "Toggle work author follow failed");
    }
}
