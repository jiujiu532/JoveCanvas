...
export async function POST(request: Request) {
    ...
    const response = await withGenerationConcurrencyLimit(user.id, "render", 60 * 60_000, renderLimit, async () => {
        ...
        if (!size) return NextResponse.json({ code: 400, data: null, msg: await serverMessage("drama.sizeInvalid") }, { status: 400 });
    ...
    return response || NextResponse.json({ code: 429, data: null, msg: await serverMessage("drama.renderConcurrencyLimit", { limit: renderLimit }) }, { status: 429 });
}
