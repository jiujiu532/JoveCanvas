...
export async function POST(request: Request, { params }: { params: Promise<{ channelId: string }> }) {
    if (!isGenerationWebhookConfigured()) return NextResponse.json({ code: 503, data: null, msg: await serverMessage("tasks.webhookSecretMissing") }, { status: 503 });
    try {
        ...
        if (!verifyGenerationWebhookSignature(rawBody, signature)) return NextResponse.json({ code: 401, data: null, msg: await serverMessage("tasks.webhookSignatureInvalid") }, { status: 401 });
        ...
        if (!channel) return NextResponse.json({ code: 404, data: null, msg: await serverMessage("tasks.webhookChannelNotFound") }, { status: 404 });
