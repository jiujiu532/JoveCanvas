...
export async function GET(_request: Request, context: Context) {
    ...
    try {
        return await workPublicationOk({ work: await getWorkPublicationForUser(user.id, (await context.params).id) });
    } catch (error) {
        return await workPublicationError(error, "获取作品详情失败", "Get user work failed");
    }
}
...
export async function PATCH(request: Request, context: Context) {
    ...
    try {
        ...
        return await workPublicationOk({ work }, "作品草稿已保存");
    } catch (error) {
        ...
        return await workPublicationError(error, "保存作品失败", "Update work publication failed");
    }
}
