...
const defaultSite: {
    ...
    friendLinks: [
        ...
        { id: "qq-vozeb-open-source", label: "JoveCanvas QQ", url: VOZEB_QQ_GROUP_URL, enabled: true },
        { id: "linux-do", label: "Linux.do", url: "https://linux.do/", enabled: true },
    ],
    socials: {
        email: { enabled: true, label: "Email", url: "" },
...
// Landing showcase content is built with next-intl keys inside the component
...
export default function HomePage() {
    ...
    const heroWorkflowItems = buildHeroWorkflowItems(t);
    const heroValueItems = buildHeroValueItems(t);
    const heroPipelineItems = buildHeroPipelineItems(t);
    const heroOutputItems = buildHeroOutputItems(t);
    const homeShowcaseItems = buildHomeShowcaseItems(t);
                ...
                <header className="landing-moon-header relative z-20 mx-auto flex h-20 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6">
                    ...
                    <nav className="landing-moon-nav hidden items-center gap-1 md:flex" onPointerLeave={() => moveNavIndicator(0)}>
                        ...
                        {landingNavigationTools.map((tool, index) => (
                            ...
                            >
                                {tLayout(`landingNav.${tool.slug}`)}
                            </Link>
                        ))}
                    </nav>
                    <div className="landing-moon-actions flex items-center justify-end gap-2">
                        <LocaleSwitcher className="landing-theme-toggle landing-moon-locale" />
...
function siteShowcaseItemsToPrompts(items: SiteShowcaseItem[] = []): Prompt[] {
    ...
    return items
        ...
        .map((item) => ({
            ...
            prompt: item.prompt,
            tags: item.tags || [],
            category: item.category || "home-showcase",
            preview: item.prompt,
            createdAt: now,
            updatedAt: now,
        }));
