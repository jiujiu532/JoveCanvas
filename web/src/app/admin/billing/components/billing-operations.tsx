...
export function BillingOperations({ initialTab = "orders", initialPaymentConfig, embedded = false, hideTabs = false }: { initialTab?: BillingTab; initialPaymentConfig?: PaymentConfigSummary; embedded?: boolean; hideTabs?: boolean }) {
    ...
    const tabOptions = useMemo(
        () => [
            ...
            { label: t("tabs.products"), value: "products" as const },
            { label: t("tabs.promotions"), value: "promotions" as const },
            { label: t("tabs.coupons"), value: "coupons" as const },
            { label: t("tabs.payments"), value: "payments" as const },
        ],
        [t],
    );
    const statusOptions = useMemo(
        () => [
            ...
            { label: t("status.paid"), value: "paid" as const },
            { label: t("status.closed"), value: "closed" as const },
            { label: t("status.canceled"), value: "canceled" as const },
    ...
    const loadProducts = useCallback(async () => {
        ...
        try {
            ...
            if (!response.ok || !payload?.products) throw new Error(payload?.error || t("loadProductsFailed"));
            setProducts(payload.products);
        } catch (error) {
            message.error(error instanceof Error ? error.message : t("loadProductsFailed"));
        } finally {
    ...
    const loadPaymentConfig = useCallback(async () => {
        ...
        try {
            ...
            if (!response.ok || !payload?.paymentConfig) throw new Error(payload?.error || t("loadPaymentConfigFailed"));
            setPaymentConfig(payload.paymentConfig);
        } catch (error) {
            message.error(error instanceof Error ? error.message : t("loadPaymentConfigFailed"));
        } finally {
    ...
    const loadDashboard = useCallback(async () => {
        ...
        try {
            ...
            if (!summaryResponse.ok || !summaryPayload?.summary) throw new Error(summaryPayload?.error || t("loadSummaryFailed"));
            if (!ordersResponse.ok || !ordersPayload?.orders) throw new Error(ordersPayload?.error || t("loadOrdersFailed"));
            setSummary(summaryPayload.summary);
            setOrders(ordersPayload.orders);
            setTotal(ordersPayload.total || 0);
        } catch (error) {
            message.error(error instanceof Error ? error.message : t("loadDashboardFailed"));
        } finally {
            setLoading(false);
        }
    }, [endDate, message, t, page, startDate, status, submittedKeyword]);
    ...
    const runOrderAction = async (order: BillingOrder, action: "complete" | "close" | "refund", reason?: string) => {
        ...
        try {
            ...
            if (!response.ok) throw new Error(payload?.error || t("orderActionFailed"));
            message.success(action === "complete" ? t("completeSuccess") : action === "close" ? t("closeSuccess") : t("refundSuccess"));
            await loadDashboard();
        } catch (error) {
            message.error(error instanceof Error ? error.message : t("orderActionFailed"));
        } finally {
            setActionOrderId("");
        }
    };
    ...
    const confirmOrderAction = (order: BillingOrder, action: "complete" | "close" | "refund") => {
        if (action === "complete") {
            modal.confirm({
                ...
                content: t("completeConfirmContent"),
                okText: t("completeOk"),
                cancelText: t("cancel"),
        ...
        let reason = "";
        modal.confirm({
            title: action === "close" ? t("closeConfirmTitle") : t("refundConfirmTitle"),
            content: (
                <Input.TextArea
                    rows={3}
                    maxLength={200}
                    placeholder={action === "close" ? t("closeReasonPlaceholder") : t("refundReasonPlaceholder")}
                    onChange={(event) => {
                        reason = event.target.value;
                    }}
                />
            ),
            okText: action === "close" ? t("closeOk") : t("refundOk"),
            cancelText: t("cancel"),
    ...
    const saveProduct = async (value: ProductFormValue) => {
        ...
        try {
            ...
            const payload = (await response.json().catch(() => null)) as { error?: string } | null;
            if (!response.ok) throw new Error(payload?.error || t("saveProductFailed"));
            message.success(t("saveProductSuccess"));
            setProductModalOpen(false);
            setEditingProductId("");
            productForm.resetFields();
            await loadProducts();
        } catch (error) {
            message.error(error instanceof Error ? error.message : t("saveProductFailed"));
        } finally {
            ...
            onOk: async () => {
                ...
                try {
                    ...
                    if (response.status === 409) {
                        modal.warning({
                            title: t("productHasOrdersTitle"),
                            content: payload?.error || t("productHasOrdersContent"),
                            okText: t("goEditDelist"),
                            onOk: () => editProduct(product),
                        });
                        return;
                    }
                    if (!response.ok) throw new Error(payload?.error || t("deleteProductFailed"));
                    message.success(t("deleteProductSuccess"));
                    await loadProducts();
                } catch (error) {
                    message.error(error instanceof Error ? error.message : t("deleteProductFailed"));
                    throw error;
                } finally {
    ...
    const columns: TableColumnsType<BillingOrder> = [
        ...
        },
        {
            ...
            dataIndex: "userId",
            width: 220,
            render: (_, order) => <AdminUserIdentity displayName={order.userDisplayName} username={order.userUsername} accountId={order.userAccountId} fallback={order.userId ? t("userUnavailable") : t("userUnbound")} />,
        },
        {
            title: t("createdAt"),
            dataIndex: "createdAt",
        ...
        <div className="space-y-3 sm:space-y-5">
            ...
            {activeTab === "orders" ? (
                <>
                    <section className="grid grid-cols-2 gap-2 sm:gap-3 md:grid-cols-2 xl:grid-cols-4">
                        <Metric title={t("metrics.paidAmount")} value={formatMoney(summary?.orders.paidAmountCents || 0)} icon={<CircleDollarSign className="size-4" />} tone="emerald" />
                        <Metric title={t("metrics.pendingAmount")} value={formatMoney(summary?.orders.pendingAmountCents || 0)} icon={<WalletCards className="size-4" />} tone="amber" />
                        <Metric title={t("metrics.paidOrders")} value={summary?.orders.paid || 0} icon={<ReceiptText className="size-4" />} tone="blue" />
                        <Metric title={t("metrics.reconciliationIssues")} value={reconciliationIssues} icon={<AlertTriangle className="size-4" />} tone={reconciliationIssues ? "rose" : "slate"} />
                    </section>
                    ...
                    >
                        <div>
                            <div>
                                <h2 className="text-base font-semibold text-stone-950 dark:text-stone-100">{t("opsFilterTitle")}</h2>
                                <p className="mt-1 text-sm text-stone-500 dark:text-stone-400">{t("opsFilterDesc")}</p>
                            </div>
            ...
            {activeTab === "products" ? (
                <>
                        ...
                        >
                            <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-x-3 gap-y-1.5 border-b border-stone-200 p-3 sm:flex sm:flex-row sm:items-center sm:justify-between sm:gap-3 sm:p-4 dark:border-stone-800">
                                <div className="contents sm:block">
                                    <h2 className="text-base font-semibold text-stone-950 dark:text-stone-100">{t("products.title")}</h2>
                                    <div className="col-span-2 line-clamp-2 text-xs leading-5 text-stone-500 sm:mt-1 sm:block sm:text-sm dark:text-stone-400">{t("products.description")}</div>
                                </div>
                                <div className="col-start-2 row-start-1 flex flex-wrap justify-end gap-1.5 sm:gap-2">
                                    <Button type="primary" aria-label={t("products.create")} title={t("products.create")} icon={<Plus className="size-4" />} onClick={openCreateProductModal}>
                                        <span className="sm:hidden">{t("products.createShort")}</span>
                                        <span className="hidden sm:inline">{t("products.create")}</span>
                                    </Button>
                                    <Button aria-label={t("products.refreshAria")} title={t("products.refreshAria")} icon={<RefreshCw className="size-4" />} loading={productsLoading} onClick={() => void loadProducts()}>
                                                    ...
                                                    {product.pricing.discountCents > 0 ? (
                                                        <div className="mt-2 flex min-w-0 flex-wrap items-center gap-1.5 text-xs">
                                                            <Tag className="m-0" color="red">
                                                                {product.pricing.promotion?.label || t("products.promoActive")}
                                                            </Tag>
                                                            <span className="font-semibold text-rose-600 dark:text-rose-300">{t("products.salePrice", { price: formatMoney(product.pricing.saleUnitAmountCents, product.currency) })}</span>
                                                            <span className="text-stone-400 line-through dark:text-stone-500">{t("products.listPrice", { price: formatMoney(product.amountCents, product.currency) })}</span>
                                                        </div>
                                                    ) : null}
                                                </div>
                                                <div className="flex shrink-0 items-center gap-1.5">
                                                    <Tag color={product.productKind === "points" ? "gold" : "blue"}>{product.productKind === "points" ? t("products.kindPoints") : t("products.kindPlan")}</Tag>
                                                    <Tag color={product.enabled ? "green" : "default"}>{product.enabled ? t("products.listed") : t("products.delisted")}</Tag>
                                                </div>
                                            </div>
                                            <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-stone-500 sm:mt-4 sm:grid-cols-4 dark:text-stone-400">
                                                <ProductFact label={t("products.listPriceLabel")} value={formatMoney(product.amountCents, product.currency)} />
                                                <ProductFact label={product.productKind === "points" ? t("products.rechargePoints") : t("products.permanentPoints")} value={`${product.pointsAmount}`} />
                                                <ProductFact label={t("products.dailyGift")} value={product.productKind === "plan" ? `${product.dailyPoints}` : "-"} />
                                                <ProductFact label={t("products.period")} value={product.productKind === "plan" ? (product.periodDays ? t("periodDaysValue", { count: product.periodDays }) : t("orderLongTerm")) : t("products.oneTime")} />
                                            </div>
                    ...
                    <Modal
                        title={editingProductId ? t("products.editTitle") : t("products.createTitle")}
                    ...
                    >
                        <Form form={productForm} layout="vertical" initialValues={defaultProductFormValue(products.length + 1)} onFinish={(value) => void saveProduct(value)}>
                            ...
                            <Form.Item name="name" label={t("products.name")} rules={[{ required: true, message: t("products.nameRequired") }]}>
                                <Input maxLength={80} placeholder={t("products.namePlaceholder")} />
                            </Form.Item>
                            <Form.Item name="description" label={t("products.descriptionLabel")}>
                                <Input.TextArea rows={3} maxLength={500} placeholder={t("products.descriptionPlaceholder")} />
                            </Form.Item>
                            <Form.Item name="productKind" label={t("products.productKind")} rules={[{ required: true, message: t("products.productKindRequired") }]}>
                                <Segmented
                                    block
                                    options={[
                                        { label: t("products.kindPlanBenefit"), value: "plan" },
                                        { label: t("products.kindPointsTopup"), value: "points" },
                                    ]}
                                />
                            </Form.Item>
                            <div className="grid gap-3 sm:grid-cols-2">
                                {productKind === "plan" ? (
                                    <Form.Item name="planId" label={t("products.planId")} rules={[{ required: true, message: t("products.planIdRequired") }]}>
                                        <Input maxLength={80} placeholder="creator / pro" />
                                    </Form.Item>
                                ) : null}
                                <Form.Item name="currency" label={t("products.currency")} rules={[{ required: true, message: t("products.currencyRequired") }]}>
                                    <Input maxLength={8} placeholder="CNY" />
                                </Form.Item>
                                <Form.Item name="amountYuan" label={t("products.amount")} rules={[{ required: true, message: t("products.amountRequired") }]}>
                                    <InputNumber min={0} precision={2} className="w-full" prefix="¥" />
                                </Form.Item>
                                <Form.Item name="pointsAmount" label={t("products.pointsAmount")} rules={[{ required: true, message: t("products.pointsAmountRequired") }]} extra={t("products.pointsAmountExtra")}>
                                    <InputNumber min={0} precision={0} className="w-full" />
                                </Form.Item>
                                {productKind === "plan" ? (
                                    <>
                                        <Form.Item name="dailyPoints" label={t("products.dailyPoints")} rules={[{ required: true, message: t("products.dailyPointsRequired") }]} extra={t("products.dailyPointsExtra")}>
                                            <InputNumber min={0} precision={0} className="w-full" />
                                        </Form.Item>
                                        <Form.Item name="periodDays" label={t("products.periodDays")} rules={[{ required: true, message: t("products.periodDaysRequired") }]}>
                            ...
                            <Form.Item name="enabled" label={t("products.enabled")} valuePropName="checked">
                                <Switch checkedChildren={t("products.listed")} unCheckedChildren={t("products.delisted")} />
                            </Form.Item>
                        </Form>
                    </Modal>
                </>
