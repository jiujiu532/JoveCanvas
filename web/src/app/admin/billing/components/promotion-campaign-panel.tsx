...
type PromoT = ReturnType<typeof useTranslations<"admin.billingOps.promotions">>;

export function PromotionCampaignPanel({ products, productsLoading }: { products: BillingProduct[]; productsLoading: boolean }) {
    const t = useTranslations("admin.billingOps.promotions");
    ...
    const showCreate = () => {
        setEditing(null);
        form.resetFields();
        form.setFieldsValue({ name: "", label: t("defaultLabel"), enabled: true, range: [dayjs(), dayjs().add(7, "day")], productIds: [], prices: {} });
        setOpen(true);
    };
    ...
    const remove = (campaign: PromotionCampaign) => {
        modal.confirm({
            title: t("deleteTitle", { name: campaign.name }),
            ...
            <div className="grid min-w-0 gap-2 p-3 sm:gap-3 sm:p-4 lg:grid-cols-2">
                {loading && !campaigns.length ? (
                    ...
                    campaigns.map((campaign) => {
                        const state = campaignState(campaign, t);
                        return (
                            <article key={campaign.id} className="min-w-0 rounded-lg border border-stone-200 bg-stone-50/70 p-3 dark:border-stone-800 dark:bg-stone-900/40 sm:p-4">
                                <div className="flex items-start justify-between gap-3">
                                    <div className="min-w-0">
                                        ...
                                        <p className="mt-2 text-xs text-stone-500 dark:text-stone-400">{t("tagLabel", { label: campaign.label })}</p>
                                    </div>
                                ...
                                <div className="mt-3 flex items-start gap-2 text-xs leading-5 text-stone-500 dark:text-stone-400">
                                    ...
                                    <span>{formatRange(campaign.startsAt, campaign.endsAt, numberLocale)}</span>
                                </div>
                                <div className="mt-3 space-y-1.5 border-t border-stone-200 pt-3 dark:border-stone-800">
                                    {campaign.products.map((item) => (
                                        <div key={item.productId} className="flex items-center justify-between gap-3 text-xs">
                                            ...
                                            <span className="shrink-0 font-semibold tabular-nums text-stone-950 dark:text-stone-100">¥ {formatYuan(item.promotionalAmountCents, numberLocale)}</span>
                                        </div>
                                    ))}
                                </div>
                ...
                <Form form={form} layout="vertical" onFinish={(value) => void save(value)}>
                    <div className="grid gap-x-3 sm:grid-cols-2">
                        ...
                        <Form.Item name="label" label={t("label")} rules={[{ required: true, message: t("labelRequired") }]}>
                            <Input maxLength={40} placeholder={t("labelPlaceholder")} />
                        </Form.Item>
                    </div>
                    <Form.Item name="range" label={t("range")} rules={[{ required: true, message: t("rangeRequired") }]}>
                    ...
                    <Form.Item className="mt-3" name="enabled" label={t("status")} valuePropName="checked">
...
function campaignState(campaign: PromotionCampaign, t: PromoT) {
    const now = Date.now();
    if (!campaign.enabled) return { label: t("stateDisabled"), color: "default" };
    if (Date.parse(campaign.startsAt) > now) return { label: t("statePending"), color: "blue" };
    if (Date.parse(campaign.endsAt) <= now) return { label: t("stateEnded"), color: "default" };
    return { label: t("stateActive"), color: "green" };
}

function formatRange(startsAt: string, endsAt: string, numberLocale: string) {
    const options: Intl.DateTimeFormatOptions = { month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", hour12: false };
    return `${new Date(startsAt).toLocaleString(numberLocale, options)} - ${new Date(endsAt).toLocaleString(numberLocale, options)}`;
}
