...
export function ReconciliationPanel({ reconciliationIssues, summary, onImport }: { reconciliationIssues: number; summary: BillingSummary | null; onImport: () => void }) {
    const t = useTranslations("admin.billingOps.elements");
    return (
        <div className="rounded-lg border border-stone-200 bg-stone-50/70 p-4 dark:border-stone-800 dark:bg-stone-900/40">
            <div className="flex items-center justify-between gap-3">
                <div className="text-sm font-semibold text-stone-950 dark:text-stone-100">{t("reconciliationTitle")}</div>
                <Tag color={reconciliationIssues ? "red" : "green"}>{reconciliationIssues ? t("needsAction") : t("normal")}</Tag>
            </div>
            <div className="mt-3 space-y-2 text-sm text-stone-600 dark:text-stone-300">
                <CheckLine label={t("paidWithoutPayment")} value={summary?.reconciliation.paidOrdersWithoutSucceededPayment || 0} />
                <CheckLine label={t("paymentWithoutOrder")} value={summary?.reconciliation.succeededPaymentsWithoutPaidOrder || 0} />
                <CheckLine label={t("amountMismatch")} value={summary?.reconciliation.amountMismatchPayments || 0} />
            </div>
...
export function ActiveProductsPanel({ activeProducts }: { activeProducts: BillingProduct[] }) {
    const t = useTranslations("admin.billingOps.elements");
    const tOps = useTranslations("admin.billingOps");
    return (
        <div className="rounded-lg border border-stone-200 bg-stone-50/70 p-4 dark:border-stone-800 dark:bg-stone-900/40">
                        ...
                        <div key={product.id} className="rounded-md border border-stone-200 bg-white p-3 dark:border-stone-800 dark:bg-stone-950">
                            ...
                            <div className="mt-2 text-xs text-stone-500 dark:text-stone-400">
                                {t("pointsWithPeriod", {
                                    ...
                                    period: product.periodDays ? tOps("periodDaysValue", { count: product.periodDays }) : tOps("orderLongTerm"),
                                })}
                            </div>
                        </div>
                    ))
                ) : (
                    <div className="rounded-md border border-dashed border-stone-200 px-3 py-6 text-center text-sm text-stone-500 dark:border-stone-800 dark:text-stone-400">{t("noActiveProducts")}</div>
                )}
            </div>
        </div>
    );
...
export function PaymentConfigPanel({ paymentConfig, loading, embedded, onRefresh, onCopy }: { paymentConfig: PaymentConfigSummary | null; loading: boolean; embedded?: boolean; onRefresh: () => Promise<void> | void; onCopy: (value: string) => void }) {
    ...
    const saveProviderConfig = async () => {
        ...
        try {
            ...
            if (!response.ok) throw new Error(payload?.error || t("savePaymentConfigFailed"));
            message.success(t("paymentConfigSaved"));
            await onRefresh();
        } catch (error) {
            message.error(error instanceof Error ? error.message : t("savePaymentConfigFailed"));
        } finally {
            setSaving(false);
        }
    };
    ...
    const readinessLabel = (ready: boolean, checkoutReady: boolean) => (ready ? t("ready") : checkoutReady ? t("awaitingWebhook") : t("awaitingConfig"));
        ...
        >
            <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-x-3 gap-y-1.5 border-b border-stone-200 px-3 py-3 sm:flex sm:flex-row sm:items-center sm:justify-between sm:gap-3 sm:px-5 sm:py-5 dark:border-stone-800">
                <div className="contents sm:block sm:min-w-0">
                    <h2 className="min-w-0 truncate text-base font-semibold tracking-normal text-stone-950 sm:text-xl dark:text-stone-100">{t("paymentConfigTitle")}</h2>
                    <div className="col-span-2 line-clamp-2 text-xs leading-5 text-stone-500 sm:mt-1 sm:block sm:text-sm sm:leading-6 dark:text-stone-400">{t("paymentConfigDesc")}</div>
                </div>
                <div className="col-start-2 row-start-1 flex flex-wrap items-center justify-end gap-1.5 sm:gap-2">
                    <Tag className="m-0 hidden sm:inline-flex">{t("readyProvidersCount", { count: paymentConfig?.readyProviders || 0 })}</Tag>
                    <Button aria-label={t("refreshPaymentStatus")} title={t("refreshPaymentStatus")} icon={<RefreshCw className="size-4" />} loading={loading} onClick={() => void onRefresh()}>
                        <span className="hidden sm:inline">{t("refreshStatus")}</span>
                    </Button>
                </div>
            </div>
            <div className="grid gap-3 bg-stone-50/70 p-3 sm:gap-5 sm:p-5 xl:grid-cols-[300px_minmax(0,1fr)] dark:bg-stone-950/45">
                <aside className="space-y-3">
                    <div className="rounded-xl border border-stone-200/80 bg-white p-2.5 sm:rounded-3xl sm:p-3 dark:border-stone-800 dark:bg-stone-900/60">
                        <div className="px-2 pb-2 text-xs font-semibold uppercase tracking-[0.14em] text-stone-400">{t("paymentChannels")}</div>
                        <div className="grid grid-cols-2 gap-1.5 sm:block sm:space-y-2">
                            {providers.map((provider) => (
                                <PaymentProviderCard key={provider.id} provider={provider} active={provider.id === activeProvider?.id} onSelect={() => setActiveProviderId(provider.id)} readinessLabel={readinessLabel} />
                            ))}
                            {!providers.length ? <div className="rounded-xl border border-dashed border-stone-200 px-3 py-10 text-center text-sm text-stone-500 dark:border-stone-800 dark:text-stone-400">{t("paymentConfigLoading")}</div> : null}
                        </div>
                    </div>
                </aside>
                {activeProvider ? (
                    <Form form={form} layout="vertical" onFinish={() => void saveProviderConfig()} className="min-w-0">
                        ...
                        <div className="min-w-0 overflow-hidden rounded-3xl border border-stone-200/80 bg-white dark:border-stone-800 dark:bg-stone-950">
                            <div className="border-b border-stone-200/80 px-3 py-3 sm:px-5 sm:py-5 dark:border-stone-800">
                                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                                    <div className="min-w-0">
                                        <div className="flex flex-wrap items-center gap-2">
                                            ...
                                            <Tag className="m-0" color={activeProvider.ready ? "green" : activeProvider.checkoutReady ? "gold" : "default"}>
                                                {readinessLabel(activeProvider.ready, activeProvider.checkoutReady)}
                                            </Tag>
                                    ...
                                    <div className="flex shrink-0 flex-wrap items-center gap-3">
                                        <Switch disabled={activeProvider.id === "manual"} checked={formEnabled} checkedChildren={t("enabled")} unCheckedChildren={t("disabled")} onChange={(checked) => form.setFieldValue("enabled", checked)} />
                            ...
                            <div className="space-y-3 p-3 sm:space-y-5 sm:p-5">
                                <PaymentFieldSection title={t("basicConfig")} description={t("basicConfigDesc")}>
                                ...
                                {advancedFields.length ? (
                                    <div className="rounded-3xl border border-stone-200/80 bg-stone-50/70 p-4 dark:border-stone-800 dark:bg-stone-900/45">
                                        ...
                                        >
                                            <span>
                                                <span className="block text-base font-semibold text-stone-950 dark:text-stone-100">{t("advancedMapping")}</span>
                                                <span className="mt-1 block text-xs leading-5 text-stone-500 dark:text-stone-400">{t("advancedMappingDesc")}</span>
                                            </span>
                                            <Tag className="m-0">{advancedOpen ? t("collapse") : t("itemCount", { count: advancedFields.length })}</Tag>
                                        </button>
                                        {advancedOpen ? (
                            ...
                            <div className="mx-5 mb-5 flex flex-col-reverse gap-3 border-t border-stone-200/80 pt-4 sm:flex-row sm:items-center sm:justify-between dark:border-stone-800">
                                <div className="text-xs leading-5 text-stone-500 dark:text-stone-400">{t("secretKeepHint")}</div>
                                <Button type="primary" htmlType="submit" icon={<Save className="size-4" />} loading={saving} className="!text-white">
                                    {t("savePaymentConfig")}
                                </Button>
                            </div>
                ...
                <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-2">
                        ...
                        <Tag className="m-0 shrink-0 !px-1 !text-[10px] sm:!px-[7px] sm:!text-xs" color={provider.ready ? "green" : provider.checkoutReady ? "gold" : "default"}>
                            {readinessLabel(provider.ready, provider.checkoutReady)}
                        </Tag>
                    </div>
...
export function PaymentConfigFieldControl({ field, providerEnabled }: { field: PaymentProviderConfigField & { configured: boolean; value?: string; sourceLabel: string }; providerEnabled: boolean }) {
    ...
    const rules = [
        {
            validator: (_: unknown, value: unknown) => {
                ...
                return Promise.reject(new Error(t("fillField", { label: field.label })));
            },
        },
    ];
    ...
    const secretPlaceholder = t("secretConfiguredPlaceholder");
    return (
        <div className={`min-w-0 self-start rounded-2xl border border-stone-200/80 bg-white shadow-sm shadow-stone-200/20 dark:border-stone-800 dark:bg-stone-950 dark:shadow-black/10 ${wide ? "p-3 md:col-span-full" : "p-3"}`}>
            ...
            <Form.Item name={field.key} rules={rules} className="m-0 !mt-2.5">
                {field.key === "mode" && field.options?.length === 2 ? (
                    ...
                    <Input.TextArea className="admin-payment-textarea" rows={field.secret ? 5 : 4} maxLength={20_000} placeholder={field.configured && field.secret ? secretPlaceholder : field.placeholder} />
                ) : field.kind === "secret" ? (
                    <Input.Password className={inputClassName} autoComplete="new-password" placeholder={field.configured ? secretPlaceholder : field.placeholder} />
                ) : (
                    <Input className={inputClassName} maxLength={2000} placeholder={field.placeholder} />
                )}
            </Form.Item>
            {field.note || usesSystemDefault ? <div className="mt-2.5 text-xs leading-5 text-stone-500 dark:text-stone-400">{field.note || t("systemDefaultNote")}</div> : null}
        </div>
    );
}
    ...
    return (
        <div className="flex min-w-0 items-center justify-between gap-2">
            <div className="min-w-0">
                ...
                {field.configured || usesSystemDefault ? <div className="mt-0.5 text-xs text-stone-400 dark:text-stone-500">{field.configured ? field.sourceLabel : t("useSystemDefault")}</div> : null}
            </div>
            <div className="flex shrink-0 flex-wrap justify-end gap-1">
                {field.required ? <Tag className="m-0">{t("required")}</Tag> : null}
                <Tag className="m-0" color={field.configured ? "green" : usesSystemDefault ? "blue" : "default"}>
                    {field.configured ? t("configured") : usesSystemDefault ? t("systemDefault") : t("notConfigured")}
                </Tag>
            </div>
        </div>
    );
...
export function RequirementGrid({ checkout, webhook, webhookOptional }: { checkout: PaymentConfigRequirement[]; webhook: PaymentConfigRequirement[]; webhookOptional?: boolean }) {
    const t = useTranslations("admin.billingOps.elements");
    const items = [...checkout.map((item) => ({ ...item, group: t("groupCheckout") })), ...webhook.map((item) => ({ ...item, group: t("groupWebhook") }))];
    if (!items.length) {
        return <div className="rounded-md border border-dashed border-stone-200 px-3 py-3 text-sm text-stone-500 dark:border-stone-800 dark:text-stone-400">{webhookOptional ? t("webhookNotRequired") : t("noRequiredVars")}</div>;
    }
    return (
                ...
                <div key={`${item.group}:${item.label}:${item.envNames.join(",")}`} className="min-w-0 rounded-xl border border-stone-200 bg-white px-3 py-2 dark:border-stone-800 dark:bg-stone-950">
                    <div className="flex items-center justify-between gap-2">
                        ...
                        <Tag className="m-0" color={item.configured ? "green" : "gold"}>
                            {item.configured ? t("setDone") : t("setPending")}
                        </Tag>
                    </div>
...
export async function copyText(value: string, message: { success: (content: string) => void; error: (content: string) => void }, labels?: { copied: string; copyFailed: string }) {
    const L = labels || { copied: "已复制", copyFailed: "复制失败" };
    try {
        await navigator.clipboard.writeText(value);
        message.success(L.copied);
    } catch {
        message.error(L.copyFailed);
    }
}
...
export function statusLabel(status: BillingOrderStatus, labels?: Partial<Record<BillingOrderStatus, string>>) {
...
export function providerLabel(value: string, labels?: Record<string, string>) {
