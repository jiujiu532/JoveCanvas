...
export function DramaShotFrameEditor({ projectId, episodeId, shot }: { projectId: string; episodeId: string; shot: DramaShot }) {
    ...
    const startFrameLabel = t("storyboard.startFrame");
    const endFrameLabel = t("storyboard.endFrame");
    ...
    const uploadFrame = async (file?: File) => {
        ...
        try {
            ...
            message.success(t("storyboard.frame.uploadedSuccess", { frame: uploadTarget === "start" ? startFrameLabel : endFrameLabel }));
        } catch (error) {
            message.error(error instanceof Error ? error.message : t("storyboard.frame.uploadFailed"));
        } finally {
            setUploading("");
        ...
        <div className="mt-3.5 border-t border-border/70 pt-3.5">
            <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex min-w-0 items-baseline gap-2">
                    <div className="shrink-0 text-sm font-semibold">{t("storyboard.frame.title")}</div>
                    <p className="min-w-0 truncate text-xs leading-5 text-muted-foreground">{t("storyboard.frame.hint")}</p>
                </div>
                <Segmented
                    ...
                    value={frameMode}
                    options={[
                        { label: t("storyboard.frame.modes.single"), value: "single" },
                        { label: t("storyboard.frame.modes.firstLast"), value: "first_last" },
                    ]}
            ...
            <div className="mt-3 grid min-w-0 gap-2.5 sm:grid-cols-2">
                <FrameSlot title={startFrameLabel} url={shot.storyboardImageUrl} loading={uploading === "start"} disabled={generationActive} onUpload={() => chooseFile("start")} onRemove={() => removeFrame("start")} />
                {frameMode === "first_last" ? <FrameSlot title={endFrameLabel} url={shot.storyboardEndImageUrl} loading={uploading === "end"} disabled={generationActive} onUpload={() => chooseFile("end")} onRemove={() => removeFrame("end")} /> : null}
            </div>
    ...
    return (
        <div className="flex min-w-0 items-center gap-2.5 rounded-md border border-border/80 bg-muted/15 p-2">
            <div className="relative aspect-video w-24 shrink-0 overflow-hidden rounded border border-border/70 bg-background">
                {url ? (
                    <Image className="!size-full !object-cover" src={imagePreviewUrl(url, 640)} alt={title} preview={{ mask: t("shared.view"), src: imagePreviewUrl(url, 1920) }} />
                ) : (
                    <button
                        type="button"
                        disabled={disabled}
                        ...
                        onClick={onUpload}
                        aria-label={t("storyboard.frame.uploadAria", { title })}
                    >
            ...
            <div className="min-w-0 flex-1">
                ...
                <div className="mt-1 flex items-center gap-0.5">
                    <Button type="text" size="small" className="!h-7 !px-1.5" loading={loading} disabled={disabled} icon={<Upload className="size-3.5" />} onClick={onUpload}>
                        {url ? t("storyboard.frame.replaceButton") : t("storyboard.frame.uploadButton")}
                    </Button>
                    {url ? <Button type="text" size="small" danger disabled={disabled} className="!size-7 !min-w-0 !p-0" aria-label={t("storyboard.frame.removeAria", { title })} icon={<Trash2 className="size-3.5" />} onClick={onRemove} /> : null}
                </div>
            </div>
        </div>
    );
