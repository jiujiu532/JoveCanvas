...
export function DramaVersionModal({ open, loading, versions, onClose, onSave, onRestore }: { open: boolean; loading: boolean; versions: DramaProjectVersion[]; onClose: () => void; onSave: () => void; onRestore: (version: DramaProjectVersion) => void }) {
    ...
    return (
        <Modal title={t("modals.version.title")} open={open} onCancel={onClose} footer={<Button onClick={onClose}>{t("modals.close")}</Button>} width={640}>
            <div className="space-y-3 pt-3">
                <div className="flex flex-col items-stretch gap-2.5 border-b border-border pb-3 text-sm text-muted-foreground sm:flex-row sm:items-center sm:justify-between sm:gap-3">
                    <span>{t("modals.version.hint")}</span>
                    <Button icon={<Save className="size-4" />} onClick={onSave}>
                        {t("modals.version.saveButton")}
                    </Button>
                </div>
                {loading ? (
                    ...
                    versions.map((version) => (
                        <div key={version.id} className="flex flex-col items-stretch gap-3 rounded-lg border border-border bg-background p-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4 sm:p-4">
                            <div className="min-w-0">
                                <div className="font-medium">{t("modals.version.versionLabel", { version: version.version })}</div>
                            ...
                            <Button className="!w-full sm:!w-auto" onClick={() => onRestore(version)}>
                                {t("modals.version.restoreButton")}
                            </Button>
                        </div>
                    ))
                ) : (
                    <Empty className="!my-6 sm:!my-10" description={t("modals.version.empty")} />
                )}
            </div>
        </Modal>
    );
...
export function DramaJianyingModal({
    ...
    return (
        <Modal title={t("modals.jianying.title")} open={open} onCancel={onClose} onOk={onExport} okText={t("modals.jianying.exportButton")} cancelText={tc("cancel")} confirmLoading={exporting}>
            <div className="space-y-5 pt-3">
                <label className="block space-y-2">
                    <span className="text-sm font-medium">{t("modals.jianying.pathLabel")}</span>
                    <Input value={path} onChange={(event) => onPathChange(event.target.value)} placeholder={t("modals.jianying.pathPlaceholder")} />
                </label>
                <label className="block space-y-2">
                    <span className="text-sm font-medium">{t("modals.jianying.versionLabel")}</span>
                    <Segmented
                        block
                        value={version}
                        options={[
...
export function DramaSubtitleModal({ open, shots, onClose }: { open: boolean; shots: DramaShot[]; onClose: () => void }) {
    const t = useTranslations("drama");
    return (
        <Modal title={t("modals.subtitle.title")} open={open} footer={<Button onClick={onClose}>{t("modals.close")}</Button>} onCancel={onClose} width={720}>
