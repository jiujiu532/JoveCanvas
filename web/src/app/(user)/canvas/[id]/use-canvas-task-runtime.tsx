...
export function useCanvasTaskRuntime({ state }: { state: CanvasPageState }) {
    ...
    const confirmStopGeneration = useCallback(
        (nodeId: string) => {
            modal.confirm({
                title: t("node.stopGeneration.title"),
                content: t("node.stopGeneration.content"),
                okText: t("node.stopGeneration.ok"),
                cancelText: t("node.stopGeneration.cancel"),
    ...
    const completeVideoTask = useCallback(
        async (nodeId: string, generationConfig: AiConfig, task: NonNullable<CanvasNodeMetadata["videoTask"]>, controller: AbortController, prompt?: string) => {
            ...
            const loggedAssetPromise = recordGenerationLog({
                ...
                source: "canvas",
                status: "success",
                title: finalPrompt.slice(0, 36) || t("node.log.videoTitle"),
                prompt: finalPrompt,
                model: modelOptionName(generationConfig.model || generationConfig.videoModel),
                summary: t("node.log.videoSummary"),
    ...
    const completeImageTask = useCallback(
        async (nodeId: string, generationConfig: AiConfig, task: NonNullable<CanvasNodeMetadata["imageTask"]> | ImageGenerationTask, controller: AbortController, prompt?: string) => {
            ...
            const uploaded = await uploadGeneratedCanvasImage(image.dataUrl, image.remoteUrl, image.serverUrl, t);
            setNodes((prev) => {
    ...
    const startAndCompleteImageTask = useCallback(
        async (nodeId: string, generationConfig: AiConfig, prompt: string, references: ReferenceImage[] = [], mask: ReferenceImage | undefined, controller: AbortController) => {
            const task = await createImageGenerationTask(generationConfig, prompt, references, mask, {
                ...
                logSource: "canvas",
                logTitle: prompt.slice(0, 36) || t("node.log.imageTitle"),
    ...
    const completeTextTask = useCallback(
        async (nodeId: string, generationConfig: AiConfig, task: TextGenerationTask, controller: AbortController, prompt?: string) => {
                ...
                prev.map((node) =>
                    node.id === nodeId
                        ? {
                              ...
                              metadata: {
                                  ...node.metadata,
                                  content: answer || t("node.log.emptyTextResult"),
                                  prompt: prompt || node.metadata?.prompt,
            ...
            return answer || t("node.log.emptyTextResult");
        },
        [t],
    );
    return {
