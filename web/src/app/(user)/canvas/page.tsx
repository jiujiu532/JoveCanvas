...
export default function CanvasPage() {
    ...
    const createAndEnter = async () => {
        ...
        setCreating(true);
        try {
            enterProject(await createProject(`JoveCanvas 画布 ${projects.length + 1}`));
        } catch (error) {
    ...
    useEffect(() => {
        ...
        void (async () => {
            try {
                const id = mode === "new" ? await createProject(`JoveCanvas 画布 ${projects.length + 1}`) : projects[0]?.id || (await createProject(`JoveCanvas 画布 ${projects.length + 1}`));
                enterProject(id);
            } catch (error) {
