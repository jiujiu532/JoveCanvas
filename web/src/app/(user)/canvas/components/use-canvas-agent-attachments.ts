    ...
    const uploadOne = useCallback(
        async (id: string, file: File) => {
            ...
            try {
                ...
                const message = error instanceof Error ? error.message : t("chat.uploadFailed");
    ...
    const addFiles = useCallback(
        async (files: FileList | File[] | null) => {
            const pending = Array.from(files || [])
                ...
                .map((file) => ({ id: nanoid(), name: file.name || t("chat.pasteImage"), url: URL.createObjectURL(file), status: "uploading" as const, file }));
