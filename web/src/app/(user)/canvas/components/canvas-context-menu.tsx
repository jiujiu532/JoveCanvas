...
export function CanvasNodeContextMenu({ menu, onClose, onDuplicate, onDelete }: { menu: ContextMenuState; onClose: () => void; onDuplicate: () => void; onDelete: () => void }) {
    ...
    return (
        ...
        >
            {menu.type === "node" ? <MenuButton icon={<Plus className="size-4" />} label={t("menu.copy")} onClick={onDuplicate} /> : null}
            <MenuButton icon={<Trash2 className="size-4" />} label={t("menu.delete")} onClick={onDelete} danger />
        </div>
    );
}
